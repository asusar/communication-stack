...
Std_ReturnType PduR_ARC_Transmit(uint32 PduId, const PduInfoType* PduInfo, uint8 serviceId);
...
Std_ReturnType PduR_ARC_TriggerTransmit(uint32 PduId, PduInfoType* PduInfo, uint8 serviceId);
BufReq_ReturnType PduR_ARC_ProvideRxBuffer(uint32 PduId, PduLengthType TpSduLength, PduInfoType** PduInfoPtr, uint8 serviceId);
BufReq_ReturnType PduR_ARC_ProvideTxBuffer(uint32 PduId, PduInfoType** PduInfoPtr, uint16 Length, uint8 serviceId);

// Prototypes for functions used locally in file PduR_Logic.c (prototypes needed to remove lint errors)
BufReq_ReturnType PduR_ARC_AllocateRxBuffer(uint32 PduId, PduLengthType TpSduLength);
BufReq_ReturnType PduR_ARC_AllocateTxBuffer(uint32 PduId, uint16 length);
BufReq_ReturnType PduR_ARC_ReleaseRxBuffer(uint32 PduId);
BufReq_ReturnType PduR_ARC_ReleaseTxBuffer(uint32 PduId);
