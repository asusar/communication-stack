		...
		)
{
 ...
 /* uint8 i; */
    ...
    {
        CanIf_Global.channelData[i].Controller_Mode = CANIF_CS_STOPPED;
...
FUNC(void,CANIF_CODE) CanIf_UnInit(void)
{
    ...
    {
        CanIf_Global.channelData[i_UnInit].Controller_Mode = CANIF_CS_UNINIT;
...
//        {
//            CanIf_SetControllerMode(channel_CanIf_InitController, CANIF_CS_STOPPED); /* CANIF092 */
//        }
...
//
//    /* Set mode to stopped */
//    CanIf_SetControllerMode(channel_CanIf_InitController, CANIF_CS_STOPPED);
//}
...
//    }
            ...
            if(To_return==E_OK)
    {
        ...
        case CANIF_CS_STARTED:   /* Figure 32 */
        {
            ...
            if (Can_SetControllerMode(canControllerId_CanIf_SetControllerMode, CAN_T_START) == CAN_NOT_OK){
                /*   To_return = E_NOT_Ok; */
                To_return=E_NOT_OK;
                break;
                /* return E_NOT_OK; */
            }
            //CanIf_Global.channelData[channel_CanIf_SetControllerMode].Controller_Mode = CANIF_CS_STARTED;
        }
        break;
        ...
        case CANIF_CS_SLEEP: /* Figure 33 */
        {
            ...
            if (Can_SetControllerMode(canControllerId_CanIf_SetControllerMode, CAN_T_SLEEP) == CAN_NOT_OK){
                To_return = E_NOT_OK;
                break;
                /* return E_NOT_OK; */
            }
            //CanIf_Global.channelData[channel_CanIf_SetControllerMode].Controller_Mode = CANIF_CS_SLEEP;
            break;
        }
     ...
     CanIf_SetPduMode(channel_CanIf_SetControllerMode, CANIF_SET_OFFLINE);
            if (Can_SetControllerMode(canControllerId_CanIf_SetControllerMode, CAN_T_STOP) == CAN_NOT_OK)
            {
                To_return = E_NOT_OK;
                break;
                /* return E_NOT_OK; */
            }
            //CanIf_Global.channelData[channel_CanIf_SetControllerMode].Controller_Mode = CANIF_CS_STOPPED;
            break;
        }
				...
				P2CONST(PduInfoType, AUTOMATIC, CANIF_APPL_CONST) PduInfoPtr)
{
    ...
    P2CONST(CanIf_TxPduConfigType,AUTOMATIC,CANIF_APPL_CONST) txEntry;

    VAR(Can_ControllerStateType,AUTOMATIC) csMode=(Can_ControllerStateType) 0;
    ...
    if (txEntry == 0)
    {
        VALIDATE((uint8)FALSE, CANIF_TRANSMIT_ID, CANIF_E_INVALID_TXPDUID);
        ret = E_NOT_OK;
    }

    if(ret==E_OK)
    {
        CanIf_Arc_ChannelIdType channel_CanIf_Transmit = CanIf_ConfigPtr->InitConfig->CanIfHthCanControllerIdArr[txEntry->CanIfTxBufferRef->HthIndex];
        		/*CanIf_ConfigPtr->InitConfig->CanIfHthCanControllerIdArr[txEntry->CanIfTxBufferRef->HthIndex];*/
        ...
        if ( (ret == E_OK) && (csMode != CANIF_CS_STARTED) ){  /* CANIF_161 */
            ret= E_NOT_OK;
        }
		...
		)
{
    ...
    VALIDATE_NO_RV((uint8)(canTxPduId < (uint16)CanIf_ConfigPtr->InitConfig->CanIfNumberOfCanTXPduIds),CANIF_TXCONFIRMATION_ID,CANIF_E_PARAM_LPDU);
      ...
      //  if (entry->CanIfUserTxConfirmation != NULL) MentorTX
        ...
        if ((CanIF_TxPDUCbkConfig_a[entry->CanIfUserTxIndex].CanIfUserTxIndication) != NULL)
        {
...
FUNC(void,CANIF_CODE)CanIf_RxIndication(
		...
		/*(uint8 Hrh, Can_IdType CanId, uint8 CanDlc, const uint8 *CanSduPtr)*/ /*S_CANIF023*/
...

{
	 /*B_W CanIf_Arc_ChannelIdType channel = 0xff;*/
	   ...
	   /* sint32 i_CanIf_ControllerBusOff ; */
	    ...
	    CanIf_SetControllerMode(channel_CanIf_ControllerBusOff, CANIF_CS_STOPPED);
